...
def run_shell_command(
    stdscr: curses.window, cwd: str, selected: dict[str, set[str]]
) -> None:
...
def main(stdscr: curses.window) -> None:
    ...
    while True:
        ...
        if key in (QUIT_KEY, 27):  # q or ESC
            ...
            run_shell_command(stdscr, cwd, selected)
        elif key == SPACE_KEY:
            ...
            sel_set = selected.get(cwd, set())
            if sel_set:
                for filepath in list(sel_set):
                    try:
                        if os.path.isdir(filepath):
                            shutil.rmtree(filepath)
                        else:
                            os.remove(filepath)
                        sel_set.discard(filepath)
                    except Exception:
                        pass
                if not sel_set:
                    selected.pop(cwd, None)
                continue
